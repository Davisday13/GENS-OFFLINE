...
export function generarEstadoCuenta({ cliente, items, pagos, empresa = {}, desde, hasta, modo = 'descargar' }) {
  ...
  let logoOk = false;
  if (empresa?.logo_url) {
    try { doc.addImage(empresa.logo_url, 'PNG', margin, y, 18, 18, undefined, 'FAST'); logoOk = true; } catch {}
  }
  ...
  if (items.length > 0) {
    ...
    autoTable(doc, {
      ...
      bodyStyles: { fontSize: 8, cellPadding: 2, textColor: GRAY_900 },
      head: [['Tipo', 'Documento', 'Fecha', 'Descripción', 'Total', 'Pagado', 'Saldo']],
      body: items.map(i => [
        i.tipo_documento, i.numero_documento, fmtDate(i.fecha_documento),
        (i.descripcion || '').substring(0, 60),
        fmtMoney(i.monto_total), fmtMoney(i.monto_pagado), fmtMoney(i.saldo),
      ]),
      didParseCell: (data) => {
        if (data.section === 'body') {
          ...
          if (data.column.index === 6) {
            ...
            if (Number(saldo) > 0) { data.cell.styles.textColor = [220, 38, 38]; data.cell.styles.fontStyle = 'bold'; }
          }
      ...
      footStyles: { fillColor: GRAY_50, textColor: GRAY_900, fontStyle: 'bold', fontSize: 8 },
      columnStyles: {
  ...
  if (pagos && pagos.length > 0) {
    ...
    autoTable(doc, {
      ...
      headStyles: { fillColor: SAVANTE_BLUE, textColor: [255, 255, 255], fontSize: 8, fontStyle: 'bold', halign: 'center', cellPadding: 2 },
      bodyStyles: { fontSize: 8, cellPadding: 2, textColor: GRAY_900 },
      head: [['Fecha', 'Recibo', 'Documento', 'Método', 'Monto']],
      body: pagos.map(p => [fmtDate(p.fecha_pago), p.numero_recibo, p.documento_referencia || '-', p.metodo_pago, fmtMoney(p.monto)]),
      didParseCell: (data) => {
        if (data.section === 'body') {
          data.cell.styles.lineWidth = { top: 0, right: 0, bottom: 0.1, left: 0 };
          data.cell.styles.lineColor = GRAY_300;
          if (data.column.index === 4) { data.cell.styles.fontStyle = 'bold'; data.cell.styles.textColor = [5, 150, 105]; }
        }
      },
      columnStyles: { 0: { cellWidth: 20, halign: 'center' }, 1: { cellWidth: 25 }, 2: { cellWidth: 'auto' }, 3: { cellWidth: 25 }, 4: { cellWidth: 25, halign: 'right' } },
      margin: { left: margin, right: margin },
    });
  }
  ...
  const filename = `EstadoCuenta-${cliente.numero_cliente || cliente.nombre}.pdf`;
  if (modo === 'blob') { return { blob: doc.output('blob'), filename }; }
  doc.save(filename);
  return { filename };
}
