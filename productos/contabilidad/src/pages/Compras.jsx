...
import { getCompras, createCompra } from '../lib/api';
...
export default function Compras() {
  ...
  const [form, setForm] = useState({ fecha: hoy(), proveedor: '', concepto: '', total: '', itbms: '' });
  ...
  const guardar = async () => {
    ...
    try {
      ...
      setModal(false);
      setForm({ fecha: hoy(), proveedor: '', concepto: '', total: '', itbms: '' });
      await cargar();
    } catch (e) {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{lista.length} compras · ${total.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-400">Sin compras registradas.</div>
        ) : (
            ...
            <tbody className="divide-y divide-gray-100">
              {lista.map((r, i) => (
                <tr key={r.id || i} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600">{r.concepto || '—'}</td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-lg bg-brand-50 text-brand-700"><ShoppingCart className="h-5 w-5" /></div>
                <div>
                  <h2 className="text-lg font-semibold text-gray-900">Nueva compra</h2>
                  <p className="text-xs text-gray-500">Registrar una compra a proveedor</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="text" value={form.proveedor} onChange={(e) => setForm({ ...form, proveedor: e.target.value })}
                    className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" placeholder="Nombre del proveedor" />
                </div>
                <div className="col-span-2">
                  ...
                  <input type="text" value={form.concepto} onChange={(e) => setForm({ ...form, concepto: e.target.value })}
                    className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" placeholder="Detalle de la compra" />
                </div>
                <div>
